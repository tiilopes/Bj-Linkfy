...
function addIshountLink(cssString) {
   ...
   try {
       originalTitleText = document.evaluate(
           ...
           null
       ).stringValue.replace(/^\s*/, "").replace(/\s*$/, "").replace(/^\s*/, "").replace(/\s*$/, "").replace(/^\s*/, "").replace(/\s*$/, "").replace(new RegExp('[ìîí]','gi'), 'i').replace(new RegExp('[Ç]','gi'), 'c').replace(new RegExp('[ÚÙÛ]','gi'), 'u').replace(new RegExp('[ÓÒÔÕ]','gi'), 'o').replace(new RegExp('[ÉÈÊ]','gi'), 'e').replace(new RegExp('[ÁÀÂÃ]','gi'), 'a');
   } catch (err) {
       return;
   }
